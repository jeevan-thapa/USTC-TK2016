...
if ($($args.Count) -ne 1) {
    ...
    if ($($args[0]) -eq "-f") {
        ...
        foreach ($f in Get-ChildItem $SOURCE_PCAP_DIR) {
            mono ./0_Tool/SplitCap_2-1/SplitCap.exe -p 50000 -b 50000 -s flow -r $f.FullName -o ./2_Session/AllLayers/$($f.BaseName)-ALL
            Get-ChildItem ./2_Session/AllLayers/$($f.BaseName)-ALL | ?{$_.Length -eq 0} | Remove-Item
#            mono ./0_Tool/SplitCap_2-1/SplitCap.exe -p 50000 -b 50000 -s flow -r $f.FullName -o ./2_Session/L7/$($f.BaseName)-L7 -y L7
